...
LEGACY_TRACKABLE_ANCHORS = (*ALL_ANCHORS, *PRESSING_ANCHORS)
TRACKABLE_ANCHORS = (*LEGACY_TRACKABLE_ANCHORS, *FRONT_BODY_ANCHORS)
KNOWN_INPUT_ANCHORS = (*LEGACY_TRACKABLE_ANCHORS, LEGACY_UPPER_BACK_ANCHOR)
...
def _base_joint_name(anchor_name: str) -> str:
  for joint in ("shoulder", "hip", "knee", "ankle"):
    if anchor_name == joint or anchor_name.endswith(f"_{joint}"):
      return UPPER_BACK_ANCHOR if joint == "shoulder" and "_" not in anchor_name else joint
  return anchor_name
...
def validate_tracking_setup(value: Any, *, duration_ms: int | None = None) -> tuple[dict[str, Any] | None, str | None]:
  ...
  if version not in {TRACKING_SETUP_VERSION, FRONT_TRACKING_SETUP_VERSION}:
    return None, "unsupported_tracking_setup_version"
  if (
    version == TRACKING_SETUP_VERSION
    and value.get("barbell_target") not in {"near_side_collar", "bar_center"}
  ):
  ...
  allowed_anchors = (
    set(FRONT_BODY_ANCHORS)
    if version == FRONT_TRACKING_SETUP_VERSION
    else set(KNOWN_INPUT_ANCHORS)
  )
  unsupported_anchor = next((name for name in anchors if name not in allowed_anchors), None)
  if unsupported_anchor is not None:
    return None, f"unsupported_{unsupported_anchor}_anchor"

  anchors = (
    dict(anchors)
    if version == FRONT_TRACKING_SETUP_VERSION
    else _normalize_anchor_map(anchors)
  )
  trackable_anchors = (
    FRONT_BODY_ANCHORS
    if version == FRONT_TRACKING_SETUP_VERSION
    else LEGACY_TRACKABLE_ANCHORS
  )
  ...
  if not normalized_anchors:
    return None, "empty_tracking_anchors"

  if (
    version == TRACKING_SETUP_VERSION
    and all(name in normalized_anchors for name in (UPPER_BACK_ANCHOR, "hip", "ankle"))
  ):
  ...
  if (
    version == TRACKING_SETUP_VERSION
    and all(name in normalized_anchors for name in ("hip", "knee", "ankle"))
  ):
    hip = normalized_anchors["hip"]
  ...
  if version == FRONT_TRACKING_SETUP_VERSION:
    for side in ("left", "right"):
      shoulder = normalized_anchors.get(f"{side}_shoulder")
      hip = normalized_anchors.get(f"{side}_hip")
      knee = normalized_anchors.get(f"{side}_knee")
      ankle = normalized_anchors.get(f"{side}_ankle")
      if shoulder and hip and ankle and not (shoulder["y"] < hip["y"] < ankle["y"]):
        return None, f"invalid_{side}_body_anchor_order"
      if hip and knee and ankle and (
        knee["y"] <= hip["y"] - 0.04 or knee["y"] >= ankle["y"] + 0.04
      ):
        return None, f"invalid_{side}_knee_anchor_order"
    return {
      "version": FRONT_TRACKING_SETUP_VERSION,
...
def fuse_partial_manual_body_tracks(
  pose_frames: list[dict[str, Any]],
  *,
  setup: dict[str, Any],
  tracking: dict[str, Any],
  ...
  body_anchor_names = [name for name in USER_BODY_ANCHORS if name in (setup.get("anchors") or {})]
  source_counts = {
  ...
  base_result: dict[str, Any] = {
    ...
    "fused_anchor_names": body_anchor_names,
  ...
  if not pose_frames or not body_anchor_names or not tracking.get("tracks"):
  ...
  def record_source(joint: str, source: str) -> None:
    if joint in source_counts and source in source_counts[joint]:
  ...
  for frame in fused_frames:
    ...
    for joint in body_anchor_names:
      track = _anchor_track(tracking, joint).get(source_index)
      if not _manual_track_is_usable(track):
        record_source("upper_back" if joint == UPPER_BACK_ANCHOR else joint, "automatic")
        continue

      landmark_key = "upper_back" if joint == UPPER_BACK_ANCHOR else joint
      landmark_name = f"{selected_side}_{landmark_key}"
      raw_landmark_name = f"{selected_side}_shoulder" if joint == UPPER_BACK_ANCHOR else landmark_name
      raw_landmark = landmarks.get(raw_landmark_name) or {}
      landmark = landmarks.get(landmark_name)
      if not isinstance(landmark, dict):
        landmark = {
          "x": float(track["x"]),
          "y": float(track["y"]),
          "z": float(raw_landmark.get("z", 0.0) or 0.0),
          "visibility": 0.0,
        }
      ...
      landmark["manual_weight"] = 1.0
      landmark["user_pinned"] = True
      landmark["accepted_source"] = manual_source
      landmark["tracking_state"] = tracking_state
      landmark["chain_valid"] = False
      landmark["visual_only"] = False
      landmark["partial_pin_assisted"] = True
      if joint == UPPER_BACK_ANCHOR:
        landmark["upper_back_anchor"] = True
        upper_back_anchor_used_count += 1

      record_source(landmark_key, "reference" if tracking_state == "reference" else "pin_guided")
      fused_count += 1
      ...
      if frame_diagnostic is not None:
        frame_diagnostic["joints"][landmark_key] = {
...
def fuse_manual_front_body_tracks(
  pose_frames: list[dict[str, Any]],
  *,
  setup: dict[str, Any],
  tracking: dict[str, Any],
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
  """Fuse partial, explicitly sided front-view anchors without selecting one body side."""
  anchor_names = [
    name
    for name in FRONT_BODY_ANCHORS
  ...
  base_result: dict[str, Any] = {
    ...
    "rejection_reasons": {},
    "coverage": tracking.get("coverage") or {},
    "upper_back_anchor_key": None,
    "upper_back_anchor_semantics": None,
    "fused_anchor_names": anchor_names,
    "upper_back_anchor_used_count": 0,
    "upper_back_anchor_coverage": 0.0,
    "pin_owned_landmark_count": 0,
    "model_divergence_accepted_count": 0,
    "body_barbell_occluder_rejection_count": 0,
    "body_pin_frames": [],
  ...
  for frame in fused_frames:
    ...
    if len(frame_diagnostics) < BODY_PIN_DIAGNOSTIC_FRAME_LIMIT:
      frame_diagnostic = {
        "source_index": source_index,
        "partial": len(anchor_names) < len(FRONT_BODY_ANCHORS),
        "front_bilateral": True,
        "joints": {},
      }

    for name in anchor_names:
      track = _anchor_track(tracking, name).get(source_index)
      if not _manual_track_is_usable(track):
        source_counts[name]["automatic"] += 1
        fallback_count += 1
        continue
      ...
      landmark.update({
        ...
        "chain_valid": True,
        "visual_only": False,
        "partial_pin_assisted": len(anchor_names) < len(FRONT_BODY_ANCHORS),
        "front_bilateral_pin": True,
      })
      source_counts[name][
        "reference" if tracking_state == "reference" else "pin_guided"
      ] += 1
      fused_count += 1
      if tracking_state == "reference":
        reference_count += 1
  ...
  *,
  ...
  for name in anchor_names:
    ...
    forward = _track_direction(
      ...
      initial_point,
      barbell=is_barbell,
      joint_name=None if is_barbell else _base_joint_name(name),
      fps=fps,
    )
  ...
  *,
  ...
  def write_upper_back_landmark(
    ...
    *,
    ...
    elif source == "kinematic_estimate":
      tracking_state = "estimated"
    ...
    landmarks[landmark_name] = {
      ...
      "chain_valid": source != "pin_visual_fallback",
      "visual_only": source == "pin_visual_fallback",
    }
    if source == "pin_visual_fallback":
      landmarks[landmark_name]["chain_failure_reason"] = "visual_fallback"
  ...
  def attach_visual_fallback(
    ...
    *,
    ...
    landmark["visual_only"] = True
    landmark["chain_valid"] = False
    landmark["chain_failure_reason"] = reason
    if "occlusion" in reason or "plate" in reason:
      landmark["occlusion_reason"] = reason
  ...
  def upper_back_kinematic_estimate(
    landmarks: dict[str, Any],
    source_index: int,
  ) -> dict[str, float] | None:
    hip_track = _anchor_track(tracking, "hip").get(source_index)
    if _manual_track_is_usable(hip_track):
      hip_point = {"x": float(hip_track["x"]), "y": float(hip_track["y"])}
      confidence = min(float(hip_track.get("confidence") or 0.0), 0.48)
    else:
      hip_landmark = landmarks.get(f"{selected_side}_hip")
      if (
        not hip_landmark
        or float(hip_landmark.get("visibility") or 0.0) < MIN_MODEL_VISIBILITY
      ):
        return None
      hip_point = {"x": float(hip_landmark["x"]), "y": float(hip_landmark["y"])}
      if point_in_barbell_occluder("hip", hip_point, source_index):
        return None
      confidence = min(float(hip_landmark.get("visibility") or 0.0), 0.48)
    return {
      "x": min(max(hip_point["x"] + reference_torso_vector["x"], 0.0), 1.0),
      "y": min(max(hip_point["y"] + reference_torso_vector["y"], 0.0), 1.0),
      "visibility": max(confidence, PIN_PERSISTENCE_CONFIDENCE),
    }
  ...
  for frame_position, frame in enumerate(fused_frames):
    ...
    if upper_back_track_point is not None:
      ...
      if upper_back_inside_occluder or model_residual > max(0.16, torso_scale * 1.15):
        ...
        upper_back_point = upper_back_kinematic_estimate(landmarks, source_index)
        upper_back_source = "kinematic_estimate" if upper_back_point is not None else "pin_visual_fallback"
        if upper_back_point is None:
    ...
    for joint in FUSED_BODY_ANCHORS:
      ...
      if not landmark:
        ...
        landmarks[f"{selected_side}_{joint}"] = {
          ...
          "chain_valid": False,
          "visual_only": True,
          "chain_failure_reason": "missing_pose_landmark",
          "visual_fallback": {
            "source": "pin_visual_fallback",
    ...
    selected_segment_ratios = {
      "torso": selected_lengths["torso"] / max(reference_lengths["torso"], 1e-5),
      "thigh": selected_lengths["thigh"] / max(reference_lengths["thigh"], 1e-5),
      "shin": selected_lengths["shin"] / max(reference_lengths["shin"], 1e-5),
    }
    for joint in (UPPER_BACK_ANCHOR, *FUSED_BODY_ANCHORS):
      landmark_name = f"{selected_side}_{joint}"
      landmark = landmarks.get(landmark_name)
      if not isinstance(landmark, dict):
        continue
      if landmark.get("manual_source") == "pin_visual_fallback":
        landmark["chain_valid"] = False
        landmark["visual_only"] = True
        landmark["chain_failure_reason"] = landmark.get("chain_failure_reason") or "visual_fallback"
        continue
      landmark["chain_valid"] = True
      landmark["visual_only"] = False
      landmark["segment_length_ratios"] = selected_segment_ratios
      landmark.pop("chain_failure_reason", None)
    if frame_diagnostic is not None:
      frame_diagnostic["chain"] = {
        "selected_side": selected_side,
        "valid": True,
        "torso_length_ratio": round(selected_segment_ratios["torso"], 3),
        "thigh_length_ratio": round(selected_segment_ratios["thigh"], 3),
        "shin_length_ratio": round(selected_segment_ratios["shin"], 3),
      }
