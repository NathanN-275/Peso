...
def main():
  ...
  parser.add_argument('--hold-file', type=Path,
                      help='Private 0600 session file; pause after creating users until stdin resumes cleanup.')
  ...
  def owned_storage_paths(bucket, user_id):
    paths = []

    def walk(prefix, depth):
      if depth > 8:
        raise RuntimeError('Test-user storage tree exceeded the cleanup depth limit.')
      _, items = request(f'/storage/v1/object/list/{bucket}', method='POST', body={
        'prefix': prefix, 'limit': 100, 'offset': 0,
      })
      if len(items) == 100:
        raise RuntimeError('Test-user storage tree exceeded the cleanup page limit.')
      for item in items:
        name = item.get('name')
        ...
        is_folder = (not item.get('id') and
                     not (isinstance(metadata, dict) and any(
  ...
  try:
    ...
    if args.hold_file:
      args.hold_file.parent.mkdir(parents=True, exist_ok=True)
      descriptor = os.open(args.hold_file, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
      with os.fdopen(descriptor, 'w') as session_file:
        json.dump({'project': PROJECT, 'run': run, 'users': users}, session_file)
      hold_created = True
      print(json.dumps({'project': PROJECT, 'user_ids': report['user_ids'],
                        'hold_file': str(args.hold_file)}), flush=True)
      input('Press Enter after beta acceptance to run isolation checks and delete both users: ')
    ...
    # Storage objects do not cascade when auth users are deleted. The held
    # beta run may create source, playback, thumbnail, and export objects.
    for user in users:
      _, reservations = request(f"/rest/v1/upload_reservations?user_id=eq.{user['id']}&select=id")
      _, videos = request(f"/rest/v1/videos?user_id=eq.{user['id']}&select=id")
      for bucket in ['videos', 'saved-lift-exports', 'profile-avatars']:
        paths = owned_storage_paths(bucket, user['id'])
        if paths:
          request(f'/storage/v1/object/{bucket}', method='DELETE', body={'prefixes': paths})
        assert owned_storage_paths(bucket, user['id']) == [], 'Test user storage objects were not cleaned up'
      if user.get('token'):
        request('/auth/v1/logout?scope=global', method='POST', token=user['token'])
      request('/auth/v1/admin/users/' + user['id'], method='DELETE')
      status, _ = request('/auth/v1/admin/users/' + user['id'], allow_error=True)
      assert status == 404, 'Test Auth user was not deleted'
      for reservation in reservations:
        request(f"/rest/v1/upload_reservations?id=eq.{reservation['id']}", method='DELETE')
      _, rows = request(f"/rest/v1/videos?user_id=eq.{user['id']}&select=id")
      assert rows == [], 'Test user rows were not cleaned up'
      for video in videos:
        for table in ['analysis_jobs', 'analysis_results']:
          _, related = request(f"/rest/v1/{table}?video_id=eq.{video['id']}&select=id")
          assert related == [], f'Test user {table} rows were not cleaned up'
      _, exports = request(f"/rest/v1/saved_lift_export_jobs?user_id=eq.{user['id']}&select=id")
      assert exports == [], 'Test user export jobs were not cleaned up'
      _, remaining_reservations = request(
        f"/rest/v1/upload_reservations?blob_path=like.{user['id']}/*&select=id"
      )
      assert remaining_reservations == [], 'Test user reservation rows were not cleaned up'
      report['cleanup'].append({'user': user['id'], 'sessions_revoked': True,
                                'user_deleted': True, 'rows_removed': True,
                                'reservations_removed': True, 'storage_removed': True})
    args.report.parent.mkdir(parents=True, exist_ok=True)
    args.report.write_text(json.dumps(report, indent=2))
    args.report.chmod(0o600)
    if hold_created:
      args.hold_file.unlink(missing_ok=True)
