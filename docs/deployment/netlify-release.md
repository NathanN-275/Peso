...
8. Set `PESO_RELEASE_ENV=student` on the stable `main` branch deploy and
...
The Netlify build runs the configuration validator before exporting. Student
builds additionally require the public API URL to match the reviewed
`config/student-api-release-binding.json` generated by the accepted Azure
deployment; the tracked pending state blocks a premature build. Run the same
...
## Permanent Student branch isolation

Only `main--peso-webapp.netlify.app` uses the Student API and the permanent
`peso-staging` Supabase project (`iseqgaewjpjcxrndibep`). Follow
[Student setup](azure-student-setup.md#8-test-website-configuration) for the
branch-specific public key, API, and challenge values. Never copy credentials
from production PesoDatabase (`jfgiydtrskpqxyorvvbc`). The production branch's
Netlify values and hosting remain unchanged. Do not activate the test website
until the image security scan and Student runtime acceptance pass.
