...
function parseNativeAuthRedirect(url) {
  ...
  const hasImplicitTokenPair = Boolean(accessToken && refreshToken);
  ...
  return {
    ...
    refreshToken,
    type,
    // Native redirects use PKCE. Token pairs are never enough to adopt a session
    // because another app or user can trigger a custom-scheme URL.
    hasSessionParams: Boolean(code),
    ...
    errorMessage: authLinkErrorMessage(destination, errorCode, errorDescription)
      ?? (hasImplicitTokenPair
        ? 'This sign-in link is no longer supported. Request a new link and try again.'
        : null),
  };
}
