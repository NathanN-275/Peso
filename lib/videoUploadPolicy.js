...
function resolveReservedUploadUrl(uploadUrl, backendUrl, uploadAuthentication) {
  const resolvedUrl = uploadUrl.startsWith('/')
    ? `${backendUrl.replace(/\/$/, '')}${uploadUrl}`
    : uploadUrl;

  if (
    uploadAuthentication === 'bearer'
    && new URL(resolvedUrl).origin !== new URL(backendUrl).origin
  ) {
    throw new Error('Authenticated upload URL must use the configured backend origin.');
  }

  return resolvedUrl;
}

function buildReservedUploadRequest(fileBody, contract, accessToken) {
  const headers = {
    ...contract.upload_headers,
    ...(contract.upload_authentication === 'bearer'
      ? { Authorization: `Bearer ${accessToken}` }
      : {}),
  };
  return {
    method: contract.upload_method,
    headers,
    body: fileBody,
  };
}
